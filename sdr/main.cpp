  ...
  while ((num_pulses < 0) || (last_pulse_num_written < num_pulses)) {
    ...
    if (rx_md.error_code != rx_metadata_t::ERROR_CODE_NONE){
      ...
      if (phase_dither) {
        ...
        transform(buff.begin(), buff.end(), buff.begin(), std::bind(std::multiplies<std::complex<float>>(), std::placeholders::_1, polar((float) 1.0 / num_presums, inversion_phase)));
      } else if (num_presums != 1) {
        // Only divide by num_presums
        transform(buff.begin(), buff.end(), buff.begin(), std::bind(std::multiplies<std::complex<float>>(), std::placeholders::_1, 1.0 / num_presums));
      }
...
void transmit_worker(tx_streamer::sptr& tx_stream, rx_streamer::sptr& rx_stream)
{
  ...
  while ((num_pulses < 0) || ((pulses_scheduled - error_count) < num_pulses))
  {
    ...
    if (phase_dither) {
      transform(chirp_unmodulated.begin(), chirp_unmodulated.end(), tx_buff.begin(), std::bind(std::multiplies<std::complex<float>>(), std::placeholders::_1, polar((float) 1.0, get_next_phase(true))));
    }
